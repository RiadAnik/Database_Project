...
while($row = mysql_fetch_array($oid,MYSQL_NUM) ) {
    $oiid=$row[0];
      $details=mysql_query("SELECT `order`.`O_id`,`product`.`P_name`,`categories`.`Cat_name`,`product`.`P_model`,`order`.`O_piece`,`O_price`
FROM `order`
INNER JOIN `product` ON `order`.`P_id`=`product`.`P_id`
INNER JOIN `categories` ON `categories`.`Cat_id`=`product`.`Cat_id` WHERE `order`.`O_id`='$oiid'");
...
<input id ="printbtn"  type="button" value="Print this page" formtarget="_blank" onclick="window.print();" >        
